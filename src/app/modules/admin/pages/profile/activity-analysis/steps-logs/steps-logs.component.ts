...
console.log(labels);
  ...
  getSteps() {
    ...
    const url = `api/PatientRegistration/GetPatientStepsGraph?patientid=${this.data.userId}&type=month`;
    this.httpService
      .getAll(url)
      .subscribe((res: any) => {
        if (res.data) {
          ...
          if (type == 1) {
            this.weekNames.forEach((weekDataObj) => {
              ...
              if (weekobj.type_name) {
                data.push({
                  x: weekobj.type_name,
                  y: weekobj.reading ? weekobj.reading : null,
                });
              } else {
                data.push({
                  x: weekDataObj,
            ...
            res.data.forEach((dataObj) => {
              if (dataObj.reading) {
                data.push({
                  x: dataObj.type_name,
                  y: dataObj.reading ? dataObj.reading : null,
                });
              }
            });
          }
