...
export class ProjectComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      this.reloadIframe();
    }, 500); // Small delay to ensure iframe is set
    ...
    console.log(this.userInfo);
    if(this.userInfo == 11912){
      this.powerBiUrl = 'https://app.powerbi.com/view?r=eyJrIjoiZjk5OGI2N2EtMTUyOS00OTc5LWE4ZjktOTAyNDg3MzM4MzQ0IiwidCI6Ijk5NGJjMDU3LTBmMmUtNGU3Ni04MjIyLTc5MjE3NWE4OWMwZSJ9'
    }else if(this.userInfo == 17647){
      this.powerBiUrl = 'https://app.powerbi.com/view?r=eyJrIjoiYTk1NjY4MDEtNjE0Ny00YjYzLThjNTctNDhhMjQ1ODQ1MmRjIiwidCI6Ijk5NGJjMDU3LTBmMmUtNGU3Ni04MjIyLTc5MjE3NWE4OWMwZSJ9';
    }else if(this.userInfo == 18364){
      this.powerBiUrl = 'https://app.powerbi.com/view?r=eyJrIjoiMzhmNzVkM2MtYTk5Ny00MjliLWE5MDktNTc3MzhiMjFjMGZmIiwidCI6Ijk5NGJjMDU3LTBmMmUtNGU3Ni04MjIyLTc5MjE3NWE4OWMwZSJ9';
    }
    ...
    this.filterSubject
      .pipe(
        ...
        map((val) => {
          if (
            this.userInfo.admin_account == 3 &&
            (this.userInfo.role_id == 6 || this.userInfo.role_id == 4)
          ) {
            this.GetRequestedPatientsInfo();
          } else {
  ...
  reloadIframe() {
    if (this.powerBiFrame) {
      this.powerBiFrame.nativeElement.src = this.powerBiUrl;
    }
  }
  ...
  newGetPatientsInfo(selectedDocorId?: any) {
    ...
    selectedDocorId = selectedDocorId
      ? selectedDocorId
    ...
    const body = {
      ...
      ismainbranch:
        this.userInfo.admin_account === this.userInfo.user_id ? true : false,
  ...
  addPatient() {
    this.dialog
      .open(AddPatientComponent, {
        ...
        data: {
          prefix_patientid: this.prefix_patientid,
          prefix_startwith: this.prefix_startwith,
        },
      })
      .afterClosed()
      .subscribe((data) => {
  ...
  getDashboardCountforAll(userId?: any) {
    const url = `api/Doctor/GetDashboardAllAppointmentCount_ByCoordinator?coordinatorid=${this.userInfo.admin_account}`;
    this.httpService.getAll(url).subscribe((res: any) => {
      this.dashBoardInfo$.next(res.data);
      console.log(this.dashBoardInfo$);
    });
  ...
  getDoctorsList() {
    ...
    this.httpService.getAll(url).subscribe((res: any) => {
      ...
      if (sessionStorage.getItem("sessionDoctorId")) {
        const filteredDoctor = this.doctors.find(
          (item) => item.doctorid == sessionStorage.getItem("sessionDoctorId")
        );
  ...
  GetRequestedPatientsInfo() {
    ...
    const url = `api/User/GetUserRequestedAppointments?pagesize=${
      this.pageSize
    }&pageno=${this.currentPage + 1}&fromdate=&todate=&searchtext=${
      this.filterVal
    }`;
    ...
    this.dialog
      .open(AppointmentFormModalComponent, {
        ...
        data: {
          ...
          page: appointmentdata.opinion_id
            ? "Enroll"
            : appointmentdata.request_status,
        },
      })
    ...
    confirmation.afterClosed().subscribe((result) => {
      if (result === "confirmed") {
        ...
        const url = `api/Doctor/UpdateAppointmentStatus?appointmentid=${
          data.appointment_id
        }&statusid=${13}&actionby=${this.userInfo.user_id}&patientid=${
          data.user_id
        }`;
        this.httpService.create(url, null).subscribe(
          (res: any) => {
  ...
  checkFolloup(data: any) {
    ...
    this.httpService.getAll(url).subscribe(
      (res: any) => {
        if (res.data) {
          this._router.navigate(["/apps/hospital-follow-up"], {
            state: {
              patientName: data.full_name,
              appointmentDate: data.next_appointment_date,
  ...
  gotoReports(data) {
    ...
    this._router.navigate(["/reports"], {
      state: { from: "Dash-board", patientId: data.user_id },
    });
  }
}
